...
<img src="C:\Users\URL\Desktop\GithubRepository\learning\2026.4.1\transReID细读day1.assets\image-20260401212112428.png" alt="image-20260401212112428" style="zoom: 67%;" />
...
注意上述分为全局分支和局部分支。局部分支由JPM提供，在下文进行讲述，重点放到全局分支（源自ViT，其实就是vison transformer中的特征提取方式）。
...
其中三元组损失函数L(triplet)，a,p,n分别代表：

- **a**：**anchor**，锚点样本。
- **p**：**positive**，正样本，与锚点样本属于同一类别（例如同一行人或同一物体）。
- **n**：**negative**，负样本，与锚点样本属于不同类别。
