...
- `data/`: 存放数据集的文件夹。
- `model/`: 模型权重保存目录（训练后创建）。
- `predict/`: 预测结果保存目录（预测后创建）。
...
1. 由于数据集规模较小，模型可能在训练集之外的数据上表现不佳。
2. 训练过程中会自动创建model目录来保存权重。
3. 预测时会自动创建predict目录来保存结果。
4. 所有超参数均在transformer.py中硬编码，如需调整请直接修改源码。