...
class LabelSmoothingLoss(nn.Module):
    ...
    def __init__(self, smoothing=0.0, num_classes=2):
        ...
        Args:
            smoothing (float): 平滑因子，控制标签平滑的程度，取值范围[0,1]
                            - 0.0: 无平滑（标准交叉熵损失）
                            - 0.1: 常用平滑值
        ...
        self.confidence = 1.0 - smoothing  # 目标类别的置信度
    ...
    def forward(self, pred, target):
        ...
        Returns:
            torch.Tensor: 计算得到的标签平滑损失值
        """
        # 对预测结果应用对数softmax，转换为对数概率
        pred = pred.log_softmax(dim=-1)
        
        # 构建平滑后的真实标签分布（不参与梯度计算）
        with torch.no_grad():
            # 创建与预测张量形状相同的全零张量
            true_dist = torch.zeros_like(pred)
            # 将所有位置填充为平滑概率值（非目标类别的概率）
            true_dist.fill_(self.smoothing / (self.num_classes - 1))
            # 将目标类别的概率设置为置信度（1.0 - smoothing）
            true_dist.scatter_(1, target.unsqueeze(1), self.confidence)
        
        # 计算交叉熵损失：-∑(真实分布 * 对数预测概率)，然后求平均
...
def warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor):
    """
    创建学习率预热调度器，用于训练初期逐步增加学习率
    
    Args:
        optimizer (torch.optim.Optimizer): 需要调整学习率的优化器
        warmup_iters (int): 预热迭代次数，在此期间学习率从低值逐渐增加到目标值
        warmup_factor (float): 预热初始因子，通常设置为较小的值（如0.1），
                              表示初始学习率是目标学习率的倍数
    ...
    # 定义学习率缩放函数
    def f(x):
        # 如果当前迭代次数超过预热期，学习率缩放因子为1（使用完整学习率）
        if x >= warmup_iters:
            return 1
        # 计算当前迭代在预热期中的进度比例
        alpha = float(x) / warmup_iters
        # 线性插值计算当前学习率缩放因子：从warmup_factor逐渐增加到1
        return warmup_factor * (1 - alpha) + alpha

    # 创建并返回基于Lambda函数的学习率调度器
