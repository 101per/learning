...
> encoder中的参考点就是自己的像素，也就是HW个，学习每个像素的K个offset
> 也就是说encoder中每个像素都和自己的K组偏移量得到的采样点做attention
>
> decoder中的参考点输入N个
>
> 最重要理解，==每一个参考点要确定K个采样点。而一个query对应一个参考点。==



4.为什么要用双线性插值法（bilinear sampling ）
...
$$
f(x,y)=
(1-\delta_x)(1-\delta_y)F_{00}
+\delta_x(1-\delta_y)F_{10}
+(1-\delta_x)\delta_y F_{01}
+\delta_x\delta_y F_{11}
$$
其中：
$$
\delta_x = x - x_0,\quad \delta_y = y - y_0
$$
梯度可以这样回传：
$$
 \frac{\partial \text{loss}}{\partial x}
=
\frac{\partial \text{loss}}{\partial f}
\cdot
\frac{\partial f}{\partial x}
$$
我的理解就是用双线性插值构造了对离散(x，y)的函数。
